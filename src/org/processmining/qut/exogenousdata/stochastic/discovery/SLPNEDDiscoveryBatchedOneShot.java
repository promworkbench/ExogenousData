...
//		System.out.println("constructed immediate choice representation");
	...
	protected Map<Function, Double> solveEquations(
		...
		int batches = (int) ((equalities.getLeft().size()/batchsize) + 1);
		log("number of batches :: "+batches+" given size of "+equalities.getLeft().size());
		ProgSetMax(ProgGetMax()+batches);
		...
		log("finished batching for one-shot equations...");
...
//	helper functions to handle progress to UI or logging
	...
	protected void log(String message) {
		System.out.println("[SLPNED-discovery-batched-one-shot] "+message);
	}
