...
public class EqualitiesProdFactory {
	
	private EqualitiesProdFactory() {};
...
//			cycle through adjustments
			...
			String ajname = knower ?
					Variables.getExogenousAdjust(fired, power) :
					Variables.getNotExogenousAdjust(fired, power);
...
//		build bottom (different)
		...
		for (Transition trans : point.getEnabled()) {
			...
			for (int i=0; i < powers.length; i++) {
				...
				String ajname = knower ? 
						Variables.getExogenousAdjust(trans, powers[i]) :
						Variables.getNotExogenousAdjust(trans, powers[i]);
