...
public class GuardPrecisionAxiom7 {
	
	Path axiomDir = Paths.get(
			"tests","src-test","resource", "axioms", "axiom 7"
	);
	...
	public void testAxiom() {
		...
		for(int i=1; i<6; i++) {
			...
			System.out.println(
					"[Axiom 7] Computed guard-precision ("
					+i
					+") was :: " 
					+ ret);
			scores.add(ret);
		...
		for(int i=0; i <scores.size()-1; i++) {
			for(int j=i+1; j < scores.size(); j++) {
				if (scores.get(i) < scores.get(j)) {
					fail(
						"[Axiom 7] The computed guard-precision for model "
						+ i 
						+ " ("
						+ scores.get(i) 
						+ ") was smaller than expected, comparing to model "
						+ j
						+ " ("
						+ scores.get(j)
						+ ")"
